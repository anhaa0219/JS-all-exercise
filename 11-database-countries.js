...
console.log(
  landlockedOnly(countries).every(function (x) {
    return x.landlocked;
  }),
);
...
function countByContinent(db) {
  return db.reduce(function (sum, db) {
    let total = db.continent;
...
function populationByContinent(db) {
  return db.reduce(function (sum, db) {
    let total = db.continent;
    sum[total] = (sum[total] || 0) + db.population;
    return sum;
  }, {});
}
exercise();
...
function densest(db) {
  return db
    .map(function (item) {
      let sum = (item.population * 1000000) / item.area;
      return { name: item.name, density: Math.floor(sum) };
    })
    .sort(function (a, b) {
